...
/*!
  @brief   Deallocate NeoPixel object, set data pin back to INPUT, unclaim the statemachine
*/
NeoPixel::~NeoPixel() {
...
void NeoPixel::rp2040Init(uint8_t set_pin)
{
...
void NeoPixel::rp2040changepin(uint8_t set_pin)
{
	pin = set_pin ;
...
void  NeoPixel::rp2040Show(uint8_t pin, uint8_t *pixels, uint32_t numBytes, bool is800KHz)
{ 
    PRINTF0("In Show,");
	if (!begun)
    {
...
void NeoPixel::setBrightnessFunctions(pBrightnessFunc fr, pBrightnessFunc fg, pBrightnessFunc fb, pBrightnessFunc fw) {
