...
static bool sd_wait_ready(sd_card_t *pSD, int timeout) {
    ...
    if (resp == 0x00) FATFS_SPI_DBG_PRINTF("%s failed\r\n", __FUNCTION__);
...
static int sd_cmd(sd_card_t *pSD, const cmdSupported cmd, uint32_t arg,
    ...
    if (CMD12_STOP_TRANSMISSION != cmd) {
        if (false == sd_wait_ready(pSD, SD_COMMAND_TIMEOUT)) {
            FATFS_SPI_DBG_PRINTF("%s:%d: Card not ready yet\r\n", __FILE__, __LINE__);
        }
    }
    // Re-try command
    for (int i = 0; i < SD_COMMAND_RETRIES; i++) {
        ...
        if (R1_NO_RESPONSE == response) {
            FATFS_SPI_DBG_PRINTF("No response CMD:%d\r\n", cmd);
            continue;
        }
        break;
    }
    ...
    if (R1_NO_RESPONSE == response) {
        FATFS_SPI_DBG_PRINTF("No response CMD:%d response: 0x%" PRIx32 "\r\n", cmd,
                   response);
        return SD_BLOCK_DEVICE_ERROR_NO_DEVICE;  // No device
    }
    if (response & R1_COM_CRC_ERROR && ACMD23_SET_WR_BLK_ERASE_COUNT != cmd) {
        FATFS_SPI_DBG_PRINTF("CRC error CMD:%d response 0x%" PRIx32 "\r\n", cmd, response);
        return SD_BLOCK_DEVICE_ERROR_CRC;  // CRC error
    }
    if (response & R1_ILLEGAL_COMMAND) {
        if (ACMD23_SET_WR_BLK_ERASE_COUNT != cmd)
            FATFS_SPI_DBG_PRINTF("Illegal command CMD:%d response 0x%" PRIx32 "\r\n", cmd,
                       response);
    ...
    //	FATFS_SPI_DBG_PRINTF("CMD:%d \t arg:0x%" PRIx32 " \t Response:0x%" PRIx32 "\r\n",
    // cmd, arg, response);
    ...
    switch (cmd) {
        case CMD8_SEND_IF_COND:  // Response R7
            FATFS_SPI_DBG_PRINTF("V2-Version Card\r\n");
        ...
        case CMD58_READ_OCR:  // Response R3
            ...
            response |= sd_spi_write(pSD, SPI_FILL_CHAR);
            FATFS_SPI_DBG_PRINTF("R3/R7: 0x%" PRIx32 "\r\n", response);
            break;
        ...
        case CMD13_SEND_STATUS:  // Response R2
            ...
            response |= sd_spi_write(pSD, SPI_FILL_CHAR);
            if (response) {
                FATFS_SPI_DBG_PRINTF("R2: 0x%" PRIx32 "\r\n", response);
                if (response & 0x01 << 0) {
                    FATFS_SPI_DBG_PRINTF("Card is Locked                         \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_WRITE;
                }
                if (response & 0x01 << 1) {
                    FATFS_SPI_DBG_PRINTF("WP Erase Skip, Lock/Unlock Cmd Failed  \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_WRITE_PROTECTED;
                }
                if (response & 0x01 << 2) {
                    FATFS_SPI_DBG_PRINTF("Error                                  \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_WRITE;
                }
                if (response & 0x01 << 3) {
                    FATFS_SPI_DBG_PRINTF("CC Error                               \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_WRITE;
                }
                if (response & 0x01 << 4) {
                    FATFS_SPI_DBG_PRINTF("Card ECC Failed                        \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_WRITE;
                }
                if (response & 0x01 << 5) {
                    FATFS_SPI_DBG_PRINTF("WP Violation                           \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_WRITE_PROTECTED;
                }
                if (response & 0x01 << 6) {
                    FATFS_SPI_DBG_PRINTF("Erase Param                            \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_ERASE;
                }
                if (response & 0x01 << 7) {
                    FATFS_SPI_DBG_PRINTF("Out of Range, CSD_Overwrite            \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_PARAMETER;
                }
                if (response & 0x01 << 8) {
                    FATFS_SPI_DBG_PRINTF("In Idle State                          \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_NONE;
                }
                if (response & 0x01 << 9) {
                    FATFS_SPI_DBG_PRINTF("Erase Reset                            \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_ERASE;
                }
                if (response & 0x01 << 10) {
                    FATFS_SPI_DBG_PRINTF("Illegal Command                        \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_UNSUPPORTED;
                }
                if (response & 0x01 << 11) {
                    FATFS_SPI_DBG_PRINTF("Com CRC Error                          \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_CRC;
                }
                if (response & 0x01 << 12) {
                    FATFS_SPI_DBG_PRINTF("Erase Sequence Error                   \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_ERASE;
                }
                if (response & 0x01 << 13) {
                    FATFS_SPI_DBG_PRINTF("Address Error                          \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_PARAMETER;
                }
                if (response & 0x01 << 14) {
                    FATFS_SPI_DBG_PRINTF("Parameter Error                        \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_PARAMETER;
                }
                break;
            }
...
static int sd_cmd8(sd_card_t *pSD) {
    ...
    if ((SD_BLOCK_DEVICE_ERROR_NONE == status) &&
        ...
        if ((response & 0xFFF) != arg) {
            FATFS_SPI_DBG_PRINTF("CMD8 Pattern mismatch 0x%" PRIx32 " : 0x%" PRIx32
                       "\r\n",
                       arg, response);
            pSD->card_type = CARD_UNKNOWN;
...
static uint64_t sd_sectors_nolock(sd_card_t *pSD) {
    ...
    if (sd_cmd(pSD, CMD9_SEND_CSD, 0x0, false, 0) != 0x0) {
        FATFS_SPI_DBG_PRINTF("Didn't get a response from the disk\r\n");
        return 0;
    }
    uint8_t csd[16];
    if (sd_read_bytes(pSD, csd, 16) != 0) {
        FATFS_SPI_DBG_PRINTF("Couldn't read csd response from disk\r\n");
        return 0;
    }
    ...
    switch (csd_structure) {
        case 0:
            ...
            FATFS_SPI_DBG_PRINTF("Standard Capacity: c_size: %" PRIu32 "\r\n", c_size);
            FATFS_SPI_DBG_PRINTF("Sectors: 0x%llx : %llu\r\n", blocks, blocks);
            FATFS_SPI_DBG_PRINTF("Capacity: 0x%llx : %llu MB\r\n", capacity,
        ...
        case 1:
            ...
            FATFS_SPI_DBG_PRINTF("SDHC/SDXC Card: hc_c_size: %" PRIu32 "\r\n", hc_c_size);
            FATFS_SPI_DBG_PRINTF("Sectors: %8llu\r\n", blocks);
            FATFS_SPI_DBG_PRINTF("Capacity: %8llu MB\r\n", (blocks / (2048U)));
            break;

        default:
            FATFS_SPI_DBG_PRINTF("CSD struct unsupported\r\n");
...
static bool sd_wait_token(sd_card_t *pSD, uint8_t token) {
    ...
    FATFS_SPI_DBG_PRINTF("sd_wait_token: timeout\r\n");
    return false;
}
...
static int sd_read_bytes(sd_card_t *pSD, uint8_t *buffer, uint32_t length) {
    ...
    if (false == sd_wait_token(pSD, SPI_START_BLOCK)) {
        FATFS_SPI_DBG_PRINTF("%s:%d Read timeout\r\n", __FILE__, __LINE__);
    ...
    if (crc_on) {
        ...
        if ((uint16_t)crc_result != crc) {
            FATFS_SPI_DBG_PRINTF("_read_bytes: Invalid CRC received 0x%" PRIx16
...
static int sd_read_block(sd_card_t *pSD, uint8_t *buffer, uint32_t length) {
    ...
    if (crc_on) {
        ...
        crc_result = crc16((void *)buffer, length);
        if ((uint16_t)crc_result != crc) {
            FATFS_SPI_DBG_PRINTF("%s: Invalid CRC received 0x%" PRIx16
                       " result of computation 0x%" PRIx16 "\r\n",
...
static int in_sd_write_blocks(sd_card_t *pSD, const uint8_t *buffer,
    ...
    if (blockCnt == 1) {
        ...
        if (response != SPI_DATA_ACCEPTED) {
            FATFS_SPI_DBG_PRINTF("Single Block Write failed: 0x%x \r\n", response);
            status = SD_BLOCK_DEVICE_ERROR_WRITE;
        }
    } else {
        ...
        do {
            ...
            if (response != SPI_DATA_ACCEPTED) {
                FATFS_SPI_DBG_PRINTF("Multiple Block Write failed: 0x%x\r\n", response);
                status = SD_BLOCK_DEVICE_ERROR_WRITE;
                break;
            }
            buffer += _block_size;
...
static int sd_init_medium(sd_card_t *pSD) {
    ...
    */
    ...
    if (sd_go_idle_state(pSD) != R1_IDLE_STATE) {
        FATFS_SPI_DBG_PRINTF("No disk, or could not put SD card in to SPI idle state\r\n");
    ...
    if ((SD_BLOCK_DEVICE_ERROR_NONE != status) || (0x00 != response)) {
        pSD->card_type = CARD_UNKNOWN;
        FATFS_SPI_DBG_PRINTF("Timeout waiting for card\r\n");
        return status;
    }
    ...
    if (SDCARD_V2 == pSD->card_type) {
        ...
        if (SD_BLOCK_DEVICE_ERROR_NONE ==
            ...
            if (response & OCR_HCS_CCS) {
                ...
                FATFS_SPI_DBG_PRINTF("Card Initialized: High Capacity Card\r\n");
            } else {
                FATFS_SPI_DBG_PRINTF(
                    "Card Initialized: Standard Capacity Card: Version 2.x\r\n");
            }
        }
    } else {
        pSD->card_type = SDCARD_V1;
        FATFS_SPI_DBG_PRINTF("Card Initialized: Version 1.x Card\r\n");
    }
...
static int sd_init(sd_card_t *pSD) {
    ...
    if (SD_BLOCK_DEVICE_ERROR_NONE != err) {
        FATFS_SPI_DBG_PRINTF("Failed to initialize card\r\n");
        sd_spi_release(pSD);
        sd_unlock(pSD);
        return pSD->m_Status;
    }
    FATFS_SPI_DBG_PRINTF("SD card initialized\r\n");
    ...
    if (sd_cmd(pSD, CMD16_SET_BLOCKLEN, _block_size, false, 0) != 0) {
        FATFS_SPI_DBG_PRINTF("Set %" PRIu32 "-byte block timed out\r\n", _block_size);
        sd_spi_release(pSD);
        sd_unlock(pSD);
        return pSD->m_Status;
    }
