...
class Maintenance:
    ...
    def __init__(self, button_pin: int=24, led_pin: int=23) -> None:
        ...
        '''
        self.button_pin = InPin(pin=button_pin, pull_up=True, bounce_time=.5) # type: ignore
        self.led_pin = Led(pin=led_pin, active_high=True) # type: ignore
    ...
    def begin_maintenance(self, callback: maintenance_cb_t) -> None:
        ...
        def wrapper():
            self.led_blink()
            callback()
        self.button_pin.when_deactivated = wrapper # type: ignore
    ...
    def led_blink(self, force: bool=False):
        if force or self._led_state != Maintenance.LedState.BLINK:
            self.led_pin.blink(on_time=0.5, off_time=0.5, fade_in_time=0, fade_out_time=0, n=None, background=True) # type: ignore
            self._led_state = Maintenance.LedState.BLINK


    @property
    def button_state(self) -> bool:
        return bool(self.button_pin.value) # type: ignore
